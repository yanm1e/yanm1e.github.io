...
![aPiOVP.png](https://s1.ax1x.com/2020/07/27/aPiOVP.png)

下载之后会有一个图形化界面Zenmap,

![aPFQq1.png](https://s1.ax1x.com/2020/07/27/aPFQq1.png)

当然命令行dos窗口也可以使用，

![aPF1Vx.png](https://s1.ax1x.com/2020/07/27/aPF1Vx.png)
...
![aPFWss.png](https://s1.ax1x.com/2020/07/27/aPFWss.png)
...
诉Nmap 永远 对目标IP地址作反向域名解析。 一般只有当发现机器正在运行时才进行这项操作。
...
当SYN扫描不能用时，TCP Connect()扫描就是默认的TCP扫描。 当用户没有权限发送原始报文或者扫描IPv6网络时，就是这种情况。 Instead of writing raw packets as most other scan types do，Nmap通过创建connect() 系统调用要求操作系统和目标机以及端口建立连接，而不像其它扫描类型直接发送原始报文。 这是和Web浏览器，P2P客户端以及大多数其它网络应用程序用以建立连接一样的 高层系统调用。它是叫做Berkeley Sockets API编程接口的一部分。Nmap用 该API获得每个连接尝试的状态信息，而不是读取响应的原始报文。
...
`--max-hostgroup选项用于说明使用最大的组`，

Nmap不 会超出这个大小。--min-hostgroup选项说明最小的组，Nmap 会保持组大于这个值。如果在指定的接口上没有足够的目标主机来满足所 指定的最小值，Nmap可能会采用比所指定的值小的组。这两个参数虽然很少使用， 但都用于保持组的大小在一个指定的范围之内。
...
转载自： [http://www.nmap.com.cn/doc/manual.shtm#3](http://www.nmap.com.cn/doc/manual.shtm#3)
